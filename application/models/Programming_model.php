...
class Programming_model extends CI_Model
{
    function get_challenges(){
        ...
        $sql = "SELECT  p.*,c.course_name,c.course_id FROM tc_programing_quiz AS p 
                ...
                WHERE e.student_id = $student_id AND e.course_id = p.course_id  ";
   ...
   function get_challenge_att_count($pq_id,$student_id){
    $sql = "SELECT DISTINCT pqc_m FROM tc_challenge_submition WHERE corr_incc =1 AND  pq_id = $pq_id AND student_id = $student_id";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return $query->num_rows();
        }else{
   ...
   function get_solved_challenge($pq_map_id,$student_id){
    $sql = "SELECT DISTINCT pqc_m FROM tc_challenge_submition WHERE corr_incc =1 AND  pqc_m = $pq_map_id AND student_id = $student_id";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            return true;
        }else{
   ...
   function get_all_submited_challenges($pqc_m,$student_id){
    $sql = "SELECT s.*, b.challenge_name,b.marks,c.course_name FROM tc_challenge_submition AS s, tc_challenge_bank AS b, tc_course AS c
           WHERE s.pqc_m = $pqc_m AND s.ch_id = b.ch_id AND b.course_id = c.course_id AND s.student_id = $student_id ORDER BY s.cs_id DESC ";
    $query = $this->db->query($sql);
    if ($query->num_rows() > 0) {
        return $query->result_array();
    }else{
